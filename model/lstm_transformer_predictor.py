...
class EnhancedLSTMTransformerModel(nn.Module):
    ...
    def __init__(self, input_dim: int, params: dict = None):
        super().__init__()
        if params is None:
            from config.etf_config import PRODUCTION_MODEL_PARAMS
            params = PRODUCTION_MODEL_PARAMS
        ...
        # 4. Transformer 编码器（每个层独立创建，不共享权重）
...
class LSTMTransformerPredictor:
    ...
    def predict_next_day(self, df: pd.DataFrame) -> Optional[float]:
        ...
        if last_data.isnull().any().any():
            # 填充 NaN（使用 .ffill()/.bfill() 替代已废弃的 fillna(method=...)）
            last_data = last_data.ffill().bfill().fillna(0)
