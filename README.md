...
> **统一量化模拟盘框架：策略可插拔，基础不动。完全自包含，零外部依赖。**
...
本项目**完全自包含**，所有代码（包括信号计算、深度学习模型）均在项目内部，不依赖任何外部项目目录。

目前内置两个策略：
| 策略 | 方法论 | 适用场景 |
|------|--------|---------|
| `indicator`（默认） | 从 97 个技术指标中动态选出最佳指标 → 扫描沪深300选股 → 每日信号 | 短线交易、指标背离 |
| `lstm` | LSTM-Transformer 深度学习模型预测 ETF 次日涨跌幅 | ETF 趋势跟踪 |
...
│
├── model/                          │ 🧠 深度学习模型
│   ├── lstm_transformer_predictor.py  LSTM-Transformer 架构 + 训练 + 预测
│   ├── feature_engineer.py           30+ 维技术指标特征
│   └── __init__.py
│
├── signals/                        │ 📊 97 个技术指标公式
│   ├── gf.py                         GFSignal 统一策略类
│   ├── gf_factors.py                 核心因子计算函数
│   └── __init__.py
│
├── strategies/                     ← 📦 策略模块（只需换这里）
│   ├── __init__.py                 ← 策略注册表
│   ├── base.py                     ← 抽象基类（定义 generate 接口）
│   ├── dynamic_indicator.py        ← 动态指标选优策略
│   └── model_lstm.py               ← LSTM ETF 预测策略
│
├── output/                         ← 📊 运行时产物
│   └── state.json                  ← 持仓/资金/状态
│
...
本项目代码完全自包含，但策略算法的来源如下：

| 策略 | 算法来源 | 移植的模块 |
|------|---------|-----------|
| `indicator` | 015_indicator_scanner | `signals/gf.py`（97指标）、`signals/gf_factors.py`（2381行因子）、`core/signal_engine.py` |
| `lstm` | 016_etf_lstm_predict | `model/lstm_transformer_predictor.py`（模型架构）、`model/feature_engineer.py`（特征工程） |

### 相对原项目的改进

| 改进项 | 原项目 | 018 |
|--------|-------|-----|
| **买入佣金** | 漏记，PnL 偏高 | 计入持仓成本，PnL 真实 |
| **资金分配** | 第1笔吃95%，后续吃残渣 | 等额分配，每只标的预算固定 |
| **回落止盈** | 定义了 `TRAILING_STOP_PCT` 但未使用 | ✅ 已实现 |
| **T+1 保护** | 无保护，当天买入可能当天止损 | `today_opened` 标记跳过风控 |
| **基准起点** | 用全量数据首末价计算 | 从模拟启动日记录基准价 |
| **持仓深拷贝** | 引用共享导致意外修改 | save 时深拷贝 |
| **架构耦合** | 策略与基础设施混在一起 | 策略可插拔，`strategies/__init__.py` 注册即可 |
| **数据源** | 015 用数据库，016 用 baostock，不统一 | 数据库优先 + baostock 兜底，统一 data_loader |

---

## ❓ 常见问题

### Q1: 项目是否依赖 015/016 项目？

**完全独立，不依赖任何外部项目。** 所有代码（包括 97 个信号指标公式和 LSTM 深度学习模型）均在项目内部。即使 015/016 目录被删除，018 仍能正常运行。

### Q2: 为什么第一次跑 indicator 策略很慢？

第一次运行需要扫描全部 97 个指标 × 300 只股票，约 2~3 分钟。之后 90 天内再运行直接走模拟盘，1 秒完成。也可以 `--strategy lstm` 跳过扫描。
...
> **写策略的人只需要关心 `generate()`，其余一切由框架保障。**

> **018_unified_trading 完全自包含，不依赖 015_indicator_scanner 或 016_etf_lstm_predict。**
